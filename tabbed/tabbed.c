...
enum { WMProtocols, WMDelete, WMName, WMState, WMFullscreen,
       XEmbed, WMSelectTab, WMClients, WMLast }; /* default atoms */
...
setup(void)
{
	...
	wmatom[WMClients] = XInternAtom(dpy, "_TABBED_CLIENTS", False);
...
updateclientlist(void)
{
	...
	if (nclients <= 0) {
		XDeleteProperty(dpy, win, wmatom[WMClients]);
		return;
	}

	windows = ecalloc(nclients, sizeof(*windows));
	for (i = 0; i < nclients; i++)
		windows[i] = clients[i]->win;
	XChangeProperty(dpy, win, wmatom[WMClients], XA_WINDOW, 32,
	                PropModeReplace, (unsigned char *)windows, nclients);
	free(windows);
}
