...
getatomprop(Client *c, Atom prop)
{
	...
	if (XGetWindowProperty(dpy, c->win, prop, 0L, sizeof atom, False,
		prop == xatom[XembedInfo] ? xatom[XembedInfo] : XA_ATOM,
		&da, &format, &nitems, &dl, &p) == Success && p) {
		if (nitems > 0 && format == 32)
			atom = *(long *)p;
		if (da == xatom[XembedInfo] && nitems > 1)
			atom = ((long *)p)[1];
		XFree(p);
	}
	return atom;
...
togglebar(const Arg *arg)
{
	...
	if (showsystray && systray) {
		XWindowChanges wc;
		wc.y = selmon->showbar ? selmon->by : -bh;
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		XChangeProperty(dpy, systray->win, netatom[NetSystemTrayOrientation], XA_CARDINAL, 32,
			PropModeReplace, (unsigned char *)&(unsigned long){SYSTEM_TRAY_ORIENTATION_HORZ}, 1);
